...
> the operator's name throughout.
>
...
| Work is being handed to a HUMAN rather than kept in-session: filing a bug/feature/finding, an agent's "Open questions" block holds real work, a thread needs to become durable work, or someone asks "is this ready / is this done" | `amini-ticket-discipline` | Same turn. Draft it, show it, get an explicit go before creating anything in the shared Linear workspace |
...
| The lesson is about HARDWARE — RTL, testbench/oracle, synthesis/area, PDK/toolchain, a tapeout contract | `silicon-design` — edit it (or its reference file) | Same turn. Shuttle post-mortems are mandatory, and arrive ~once a year. |
| An agent is about to generate RTL | `silicon-design` — and scope the agent to `src/` ONLY | Before dispatch. An agent that can edit `model/` or `test/` has no test; a retry loop on verifier output teaches it to defeat the verifier. |
...
growing. One row per round, append-only. the operator can read it whenever
...
- Human signal received that the subordinate cannot interpret
  ("the operator said X but my charter says Y")
...
uplinks (CGNAT, KPLC dips, a DGX Spark on a remote site).
...
session where a laptop and the GPU host each had 3-4 unpushed commits past
origin:
...
whose final step was "smoke jupyter-gpu on the GPU host," the parent
ran:
